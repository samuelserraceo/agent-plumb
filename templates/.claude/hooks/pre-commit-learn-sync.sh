#!/usr/bin/env bash
# SDD PreToolUse hook.
# When the active feature is in SHIP phase and the commit touches substantive SHIP content (lessons + verification)
...
# does that on its own and pre-populates both files; this hook catches *subsequent* SHIP commits.
...
# Is this commit adding content to the SHIP section (lessons + verification) of spec.md?
# We check: does the staged diff of spec.md include additions between "## PHASE: SHIP" and the next "## PHASE:" (or EOF)?
diff_spec=$(git diff --cached -- "$spec" 2>/dev/null || echo "")
ship_additions=$(printf '%s\n' "$diff_spec" | awk '
  /^@@/ { hunk=1; next }
  hunk && /^\+## PHASE: SHIP/ { in_ship=1; next }
  hunk && /^\+## PHASE:/ && !/SHIP/ { in_ship=0 }
  hunk && / ## PHASE: SHIP/ { in_ship=1; next }
  hunk && / ## PHASE:/ && !/SHIP/ { in_ship=0 }
  in_ship && /^\+[^+]/ && !/^\+\s*$/ && !/^\+<!--/ { count++ }
  END { print count+0 }
...
[ "$ship_additions" -eq 0 ] && exit 0

# SHIP content (lessons + verification) being added — enforce the sync.
...
if [ ${#missing[@]} -gt 0 ]; then
  {
    echo "[SDD] SHIP commit blocked — institutional memory must be updated in the same commit."
    echo ""
    echo "  Feature: $active"
    echo "  Phase:   SHIP"
    echo ""
