...
|---|---|
| "Yes — full browser tests" | `Test runner: Playwright` + `Viewports: Desktop (Chromium) + iPhone-13 mobile` + `Test file pattern: .sdd/features/<id>/tests/task-*.spec.ts`. Offers to run `bash extensions/playwright/enable.sh` to install the Playwright extension and set up everything for you. |
