...
## Honest caveats (current as of v1.4.1)
...
Currently at **v1.4.1** — knowledge-graph foundation + 4 doctrine playbooks (`feature` / `project` / `bug` / `refactor`) + branch-derived active resolution + CI graph-integrity gate + cost-bounded Playwright explorer + claims-audit harness + mechanical test-first hook + `sdd-migrate.sh` for updatable installs. Hardened through:
