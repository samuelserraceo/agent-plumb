...
## 2026-05-11T08:47:48Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/proposed-approach

§5 Proposed approach approved by Sam — **Approach A: per-file budget map** in `templates/.sdd/config.md` under `parameters.injection.per_file_budget_chars`. Each corpus file (INDEX/spec/principles/stack/data-model/patterns) gets its own char allocation; the hook truncates each file individually against its budget and appends a sentinel `[truncated to <N> bytes per per-file budget — re-read with the Read tool ...]` when truncation happens. Existing `cap_total_chars: 16000` stays as a defensive safety-net floor on combined output. Recommended defaults: INDEX=3000, spec=5000, principles=2000, stack=3000, data-model=3000, patterns=4000 (declared total 20000). Closes idea 003's full-reorder prerequisite (#228 shipped the INDEX-live filter as the partial win). Rejected: (B) raising the env-var cap (Sam's explicit "no cheap relief" constraint), (C) head-truncate patterns.md (chronological order makes head-truncation worse), (D) Tier 3 LLM-synthesis enablement (separate decision). Approval constraint Sam attached: "no theatre" — every numerical / enforcement claim in §5 carries either `{verify-by: T-NNN}`, `{best-effort: <who>}`, or `{prod-only: <why>}` annotation, verified by `lint-no-theatre.sh` exit 0. Section hash `3414bfe7…` written to `verification.json.approved_sections.proposed-approach`. 4 moving parts (config block, hook rewrite, sentinel marker, resolver helper) + 3 key technical choices (config location, sentinel text, cap-as-floor) acknowledged. Deferred to follow-up features: idea 003 full reorder, Tier 3 enablement, per-file truncation direction.

## 2026-05-11T08:55:25Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/data-contract

§6 Data contract approved by Sam. No new project-state entities — the feature is pure framework-config plumbing. One new framework entity to be added to `.sdd/data-model.md` during BUILD: **InjectionBudget** — a config block under `parameters.injection` analogous to Tier3Config, with two fields: `cap_total_chars` (existing safety-net floor) + `per_file_budget_chars` (new map keyed by corpus-file basename → char-count budget). Read by `templates/.claude/hooks/user-prompt-submit.sh` via `templates/.sdd/scripts/resolve-parameters.sh`. Wire shape declared as YAML block under `parameters.injection.per_file_budget_chars` with 6 default entries (INDEX=3000, spec=5000, principles=2000, stack=3000, data-model=3000, patterns=4000). 4 data-layer edge cases asked-and-answered: (1) partial override merges declared key + defaults; (2) unknown basename key falls back to documented default; (3) `null` or omitted block → all framework defaults; (4) sum > cap → per-file truncation first, then total-cap clip handles sum-overshoot. No new relations — reuses existing Hook → resolve-parameters.sh wiring. Section hash `bb14e080…` written to `verification.json.approved_sections.data-contract`. Same anti-theatre constraint as §5: each numerical / behavior claim in §6 carries a verifier annotation.

## 2026-05-11T08:58:28Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/flows

§7 Flows approved by Sam. 2 critical flows drafted, each mapped to user stories. **Flow 1: Hook injection with per-file budgets** (covers stories 1 + 3) — `user-prompt-submit.sh` reads budgets via `resolve-parameters.sh`, iterates corpus order (INDEX live-filtered first, then spec/principles/stack/data-model/patterns), applies each file's char budget independently with sentinel-on-truncation, concatenates, then applies defensive `cap_total_chars` floor on combined output. **Flow 2: Downstream override** (covers story 2) — Lucia edits her project's `config.md` with `per_file_budget_chars: {patterns: 2000, data-model: 8000}`; resolver merges (declared keys win, unspecified fall back); hook applies merged map per Flow 1. Each flow has an explicit "what's different from today's hook" callout (today: single-cap end-truncate that drops patterns.md mid-paragraph; new: per-file truncation independent of file ordering). Each acceptance signal carries a `{verify-by: T-NNN ...}` annotation. Section hash `d52fd79b…` written to `verification.json.approved_sections.flows`. lint-no-theatre.sh exit 0.

## 2026-05-11T09:05:03Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/dependencies

§8 Dependencies approved by Sam. **Zero new external services, zero new local dependencies, zero marginal cost.** Pure local bash + Python (YAML parser) — reuses what `resolve-parameters.sh` already pulls in. Existing deps used: bash (>=3.2), python3+stdlib, awk (already used by #228 INDEX live-filter), head/wc. Zero new optional deps — no PyYAML, no `yq`, no `jq` upgrades; the resolver's existing inline-Python YAML walker handles the new `per_file_budget_chars` map at the same nesting depth as existing `parameters.mcp.tier3.*` keys. Pricing math: $0 marginal cost; the hook reads local files and writes concatenated output, no billable surface. Scaling: O(6) files × O(budget chars) per UserPromptSubmit event, no fanout, no recursion. External-service exposure: none — does not touch `parameters.mcp.*` blocks (Tier3, semantic_search, Playwright-explorer); only the `parameters.injection.*` block is edited. Cost claim carries `{verify-by: code review}` + `{best-effort: Sam at SHIP}`. Scaling claim carries `{verify-by: T-NNN micro-benchmark}`. Section hash `b3a95d8b…` written to `verification.json.approved_sections.dependencies`. lint-no-theatre.sh exit 0.

## 2026-05-11T09:06:51Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/out-of-scope

§9 Out-of-scope approved by Sam. 4 explicit deferrals locked: (1) **Idea 003 full stable-first cache reorder** — the bigger speed win this feature unblocks; ships separately once 011 lands; (2) **Tier 3 LLM-driven synthesis enablement** — separate decision on latency / cost / accuracy trade-offs; (3) **Per-file truncation direction (head vs tail)** — additive schema change; revisit when a downstream user asks; (4) **Token-aware budgets** — char-counting is provider-agnostic and bash-native; token-counting needs per-model tokenizers; revisit if empirical char→token ratio proves too loose. Each deferral has a name + reason + verifier annotation. Section hash `4aba385f…` written to `verification.json.approved_sections.out-of-scope`. lint-no-theatre.sh exit 0.

## 2026-05-11T09:39:49Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/non-functional

§10 Non-functional approved by Sam. 8 constraints across 3 categories. **Performance** — NF1: hook latency budget bounded `{verify-by: T-NNN micro-benchmark}`; NF2: no regression vs today's hook `{verify-by: T-NNN side-by-side timing}`; NF3: determinism — byte-identical output on identical input `{verify-by: T-NNN diff-empty}`. **Security** — NF4: no new network surface `{verify-by: T-NNN grep test on hook diff}`; NF5: `[PROJECT DATA]` framing marker preserved `{verify-by: T-NNN}`; NF6: read-only — hook does not modify `.sdd/*` files `{verify-by: T-NNN}`. **Compliance** — NF7: no new PII surface `{best-effort: Sam at SHIP}`; NF8: no new licensed code `{verify-by: code review}`. Section hash `0a38944a…` written to `verification.json.approved_sections.non-functional`. lint-no-theatre.sh exit 0.

## 2026-05-11T09:44:11Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/acceptance-criteria

§11 Acceptance criteria approved by Sam. 14 ACs (AC1-AC14) drafted, each with `{verify-by: T-NNN}` annotation. T220-T233 reserved one-per-AC. **Config layer:** AC1 (config map sums to 20000), AC5 (`cap_total_chars` stays as floor), AC6 (`InjectionBudget` entity in data-model.md). **Hook layer:** AC2 (hook reads + applies per-file budgets independently), AC3 (sentinel emitted with byte-count substitution), AC10 (sum-overshoot: per-file first then total clip), AC11 (multi-file overflow: each file present + sentinel), AC14 (`[PROJECT DATA]` framing preserved). **Resolver layer:** AC4 (override + default), AC7 (partial override), AC8 (unknown key → documented default), AC9 (null / omitted → defaults). **Cross-cutting:** AC12 (determinism), AC13 (no new network surface). Coverage check table maps every claim from §1 / §5 / §6 / §7 / §10 to at least one AC. NF1/NF2/NF6/NF7/NF8 self-verify via inline annotations. Section hash `9f30cbff…` written to `verification.json.approved_sections.acceptance-criteria`. lint-no-theatre.sh exit 0.

## 2026-05-11T09:53:31Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/plan-decompose

§14 Plan-decompose approved by Sam. 14 BUILD tasks (T220-T233), one-per-AC, test-first per task. **5 sequential cores:** T220 (config map) → T223 (resolver extension) → T221 (hook per-file loop) → T222 (sentinel) + T224 (cap floor). **3 parallel waves per F010 wave-execution doctrine:** W1 = T225 alone (data-model.md entity, fully independent); W2 = T226/T227/T228 (resolver edge cases — partial override, unknown key, missing block — all depend only on T223); W3 = T229/T230/T231/T232/T233 (integration tests on the hook — sum-overshoot, multi-file overflow, determinism, no-network, framing preserved — all depend on T221+T222+T224). Wave-marks give ~9 of 14 tasks the option to dispatch in parallel via the Agent tool. Each task lists AC mapped, dependencies, file paths touched (test file + code file). Section hash `faab6b5f…` written to `verification.json.approved_sections.plan-decompose`. lint-no-theatre.sh exit 0.

## 2026-05-11T10:02:33Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/edge-case-sweep

§15 Edge-case sweep — 8 candidates drafted across 3 applicable categories (empty / max / bad-input). Network / concurrency / auth / mobile / time-based all N/A. Sam picked **#4 (1-byte-over boundary)**, **#5 (UTF-8 char boundary backoff)**, **#7 (negative-budget clamp)** — added as **AC15-AC17** with **T234-T236**. **AC15:** `<budget> + 1` chars → sentinel `<N> = 1` `{verify-by: T234}`. **AC16:** budget falls inside multi-byte UTF-8 char → backoff to previous clean char boundary (at most 3 trailing bytes dropped); output stays valid UTF-8 `{verify-by: T235}`. **AC17:** negative value in `per_file_budget_chars.<key>` → resolver clamps to 0 + stderr warning naming the key; hook treats as empty body + sentinel `{verify-by: T236}`. **Dropped (regression-only or specified-behavior):** #1 empty corpus file (today's no-op), #2 missing corpus file (today's skip), #3 exactly-at-budget (no sentinel), #6 zero budget (specified as empty body + sentinel in AC11/AC17 path), #8 unknown basename in config (no-op). §11 and §14 re-approved in the same step to absorb the additions (17 ACs / 17 tasks total). New §11 hash `bfa3c6d1…`; new §14 hash `73508d71…`. Wave-mark recount: W1 = 1 task, W2 = 4 tasks (added T236), W3 = 7 tasks (added T234, T235). ~12 of 17 tasks now dispatchable in parallel. lint-no-theatre.sh exit 0 throughout (one rewording: `≤3` → "at most 3" in T235 row).
...
## 2026-05-11T09:55:00Z  [[011-auto-advance-agent-led-steps]]  feature/data-contract-correction
CORRECTION (CR cycle-1 #1) to the §6 data-contract approval entry above. The original text said: "destructive actions like mark-shipped keep the approve gate even at Full per §11 destructive-actions list" — conflating two independent mechanisms. The accurate model: under `most`, the §11 destructive-actions list gates destructive actions regardless of their per-action flag; under `full`, the SAME actions are gated by their per-action `requires_user_approval: true` frontmatter (because mark-shipped, manifest-repin commits, etc. are already classified `true` in v1.6.0's matrix lock). Two independent guards covering the same surface, both correct under their own tier. The §11 destructive-actions list does NOT trigger gating under Full — it's the Most-tier safety mechanism. spec.md §6 (L98 + L106) re-worded in this same commit; the original approval-hash entry above (line 464-465) remains unchanged per the append-only contract and this entry serves as the canonical record of the corrected interpretation. Re-references: `parameters.automation.level` (the config field), `AutomationLevel` (the framework concept name), §11 (`destructive_actions` list under `parameters.automation`), and the per-action `requires_user_approval` frontmatter as the second guard.

## 2026-05-11T09:08Z  [[012-promote-legacy-queued-sh-migrator-for-pre-v1-5-2-phase-state-drift-closes-206]]  feature/proposed-approach
Sam approved §5 proposed-approach: ship `bash .sdd/scripts/promote-legacy-queued.sh` as a one-shot opt-in migrator for pre-v1.5.2 projects. Walks `.sdd/{features,bugs,refactors}/`, skips cold (`.shipped`) items, flips spec.md PHASE from `SPEC` to `QUEUED` AND canonicalises the matching INDEX.md row to `(scaffolded, PHASE: QUEUED)` when the row text already says queued/backlog. Stages but doesn't auto-commit. Two-file ship (live + templates copy, both manifest-tracked) plus a one-paragraph CLAUDE.md doc update naming the migrator + when to run it (after `sdd-migrate.sh --apply`). Alternatives rejected: minimum-diff (leaves INDEX drifted) + auto-run-on-migrate (state-machine changes should be explicit opt-in per the framework's discipline).
Hash: 0b20debac374693dd4b57d54c6479c845615fcb23b7b79af10311baece8da469

## 2026-05-11T09:12Z  [[012-promote-legacy-queued-sh-migrator-for-pre-v1-5-2-phase-state-drift-closes-206]]  feature/acceptance-criteria + plan-decompose
Sam approved §11 ACs (AC1-AC5: happy-path migration, false-positive guard, cold-feature skip, partial-state warn, idempotence) and §14 tasks (T01-T06: 5 task-tests + T06 doc paragraph). Phase advance SPEC -> BUILD locked in same commit; **Run mode:** full-autonomous (Sam's documented default per memory). v1.7 patch line target (v1.7.2 candidate alongside the v1.7.0 hook-merge-exception + v1.7.1 hash-bold-strip pair).

## 2026-05-11T09:12Z  [[012-promote-legacy-queued-sh-migrator-for-pre-v1-5-2-phase-state-drift-closes-206]]  feature/phase-advance
SPEC -> BUILD. §0 brief recorded, §1 problem captured (3 facets), §3 user-stories (2 personas), §4 ux-brief (backend stdout shape, no HTML), §5 proposed-approach APPROVED, §6 data-contract (no new entities), §7 flows (1 flow), §8 deps (zero new), §9 out-of-scope (3 deferrals), §10 non-functional (idempotent + non-destructive), §11 ACs (5), §12 sign-off (2 smokes), §13 wireframe SKIPPED, §14 tasks (6), §15 edge-case-sweep (4 EC). BUILD section appended. Active blocker: §14 T01 (script body + manifest repin).

## 2026-05-11T09:25Z  [[012-promote-legacy-queued-sh-migrator-for-pre-v1-5-2-phase-state-drift-closes-206]]  feature/mark-shipped
F012 promote-legacy-queued.sh migrator shipped via PR #237 / tag `v1.7.2`. Closes #206 (pre-v1.5.2 PHASE state drift). One-shot opt-in script that flips spec.md PHASE + canonicalises INDEX rows atomically; stages via `git add` but does NOT auto-commit; idempotent. 5 BUILD task-tests T01-T05 (happy-path / false-positive guard / cold-feature skip / partial-state warn / idempotence) all GREEN. T06 doc paragraph in templates/CLAUDE.md "Multi-feature parallel work" section. Cycle-1 CR clean. Completes the v1.7 maintenance trio (v1.7.0 hook merge-exception #220 + v1.7.1 hash bold-strip #197p2 + v1.7.2 legacy-queued migrator #206).

## 2026-05-11T10:53:00Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/cross-branch-merge-from-main

Merge of `origin/main` into `feat/per-file-injection-budgets` mid-SHIP. Main shipped v1.7.1 (#233 hash-section.sh bold-strip, F011-named) and v1.7.2 (#237 promote-legacy-queued.sh migrator, F012) between this branch's scaffold and SHIP. **ID-collision artefact:** both branches scaffolded as F011 in parallel — main's was "next-action-sh-regex-stalls-on-bold-ac-labels", this branch's is "per-file-injection-budgets-in-user-prompt-submit-hook". Wiki-links use the full folder slug so `[[011-per-file-...]]` and `[[011-next-action-...]]` are unambiguous graph nodes; both 011-* folders coexist on disk. Same pattern as F008/F009 cross-branch collision documented in PR #218 — the slug is preserved (not renamed to F013) so existing wiki-links in this branch's decisions.md entries (entries 08:47Z through 10:02Z above) continue to resolve. Conflicts resolved: INDEX.md kept Active F011-per-file (this branch) + Shipped block from main (incl. F011-hash-fix, F010 wave-execution, F012 migrator). decisions.md appended main's 5 entries chronologically AFTER this branch's 9 entries (preserves byte-prefix from this branch's HEAD per v1.7.0 append-only doctrine; main's entries are well-formed additions on top). data-model.md kept the new InjectionBudget entity (this branch) — main did not edit data-model.md in conflicting positions.

## 2026-05-11T09:40Z  [[013-playwright-explore-ship-step-needs-first-class-skip-mechanism-closes-202]]  feature/mark-shipped
F013 playwright-explore skip mechanism shipped via PR #240 / tag `v1.7.3`. Closes #202 (12-line inline justification when MCP missing). Documentation-only fix: `templates/.sdd/actions/playwright-explore.md` gains `skip_when:` frontmatter field (condition + detect + log_line keys) + `## When to skip` body section above the example block telling the agent to emit the canonical single-line skip log verbatim. 3 BUILD task-tests T01-T03 GREEN (frontmatter shape / log-line content / body-section presence). Cycle-1 CR clean. **v1.7 four-pack complete:** v1.7.0 (#220 hook merge-exception) + v1.7.1 (#197p2 hash bold-strip) + v1.7.2 (#206 legacy-queued migrator) + v1.7.3 (#202 playwright-explore skip).

## 2026-05-11T12:14:53Z  [[011-auto-advance-agent-led-steps]]  feature/mark-shipped
SHIPPED via PR #236 (squash-merge `cbf660d`). v1.8.0 candidate (Sam to tag). Closes the SPEC+SHIP auto-advance gap: `parameters.automation.level` (full|most|checkpoint, default checkpoint) + destructive_actions enumeration in config.md + 008-automation-level setup brick + `/sdd-config automation <tier>` subcommand + 3-way decision tree prose in `/next`. Documentation-shape — zero runtime daemons, zero new hooks; agent reads doctrine + config each turn. Backwards-compat: existing projects without the field keep checkpoint. Pairs with F008 (multi-model) + F010 (parallel waves) to deliver drop-the-brief-walk-away. Six CR review cycles closed (~20 actionable findings closed; 1 declined per byte-prefix append-only contract on decisions.md MD022 — same precedent as F009 cycle-3 #11 + F010 cycle-1 #6). Bonus fix landed in same PR: `pre-commit-test-first.sh` got the v1.7.0 merge-mode skip that `pre-commit-rules.sh` already had (closes a #220 follow-up gap). 7 BUILD task-tests T300-T306 GREEN + 218/218 framework tests GREEN through every CR cycle + every merge of main (v1.7.2 + v1.7.2-shipped + v1.7.3). Single canonical destructive-gate rule per tier locked in cycle-4 (Full reads only requires_user_approval; Most reads only the §11 destructive_actions list). The v1.8 trio (v1.6 multi-model + v1.7 parallel waves + v1.8 auto-advance) is now complete; the framework can drop-the-brief, dispatch waves in parallel, and auto-advance technical AGENT-LED steps in SPEC + SHIP without human approve-clicks on every step.

## 2026-05-11T12:53Z  [[014-user-prompt-submit-queries-mcp-for-context-slice-instead-of-full-notebook-inject]]  feature/mark-shipped
F014 MCP-doctrine sentinel shipped via PR #243. Pairs with F011 (#236) to form the **v1.8.0 anchor** (2-PR ship; tag covers both commits cbf660d + ecad4f1). F014 adds a conditional sentinel inside `[FRAMEWORK INSTRUCTIONS]` in user-prompt-submit.sh — fires when `parameters.mcp.enabled: true` (flat OR nested YAML), names the 5 graph queries (`get_backlinks` / `get_neighbours` / `get_pattern` / `get_references` / `search_within`), preserves legacy line otherwise. Doctrine half of #210; mechanical substitution half deferred to `feat/per-file-injection-budgets` worktree. 1 BUILD task-test T01 GREEN with 4 sub-assertions (sentinel-fires + 3 query names + sentinel-inside-trust-block + no-false-positive). Mid-flight rebase resolved INDEX.md conflict with F011's parallel ship; cycle-0 CR (auto-merge no findings).

## 2026-05-11T13:11:00Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/cross-branch-merge-from-main-cycle3

Second merge of `origin/main` into `feat/per-file-injection-budgets` mid-SHIP. Main shipped v1.7.3 (#240/#241 F013 playwright-explore skip) + v1.8.0 (#236 F011 auto-advance) + v1.8.1 (#243 F014 MCP-doctrine sentinel) since the prior merge. **Three F011's now coexist** on the repo (per-file-injection-budgets, auto-advance-agent-led-steps, next-action-sh-regex-stalls-on-bold-ac-labels) — wiki-link slugs disambiguate per the established F008/F009 collision precedent. INDEX.md "Active" line kept pointing at this branch's F011 (Sam approved this in earlier session by typing `pick 4 5 7` to extend the original 14 ACs). decisions.md preserves my branch's entries above, then main's 4 new entries (F013/F011-auto-advance/F014/F012). `templates/.sdd/config.md` auto-merged — main added `parameters.automation:` block (from F011 auto-advance/#236) AND my branch added `parameters.injection:` block (this feature); both new blocks coexist cleanly. `templates/.claude/hooks/user-prompt-submit.sh` auto-merged — main added the MCP-doctrine sentinel inside `[FRAMEWORK INSTRUCTIONS]`; my branch added the per-file budget loader + `emit_with_budget` Python helper; both changes touch different parts of the file. No data-model.md conflicts.

## 2026-05-11T18:36:00Z  [[011-per-file-injection-budgets-in-user-prompt-submit-hook]]  feature/final-contract-correction

**Final-shipped contract correction** (per CR cycle 5 MAJ #5 — decisions.md append-only). The §5/§6/§7 approval entries above froze the PRE-fix contract into the audit log. The actually-shipped contract differs in 4 places after CR cycles 1-4 reshaped the design. Recording the deltas here so the audit trail matches what the hook actually does:

1. **Resolver script name:** `resolve-parameters.sh` (entries above) → **`get-injection-budget.sh`** (shipped). A new peer helper to resolve-parameters.sh was created instead of extending the existing resolver — cleaner separation of concerns; the hook itself reads `config.md` via inline Python (one subprocess per turn vs 6 helper calls).

2. **Budget unit:** "char-count budgets" (entries above) → **byte-count budgets** (shipped, semantic). The config key name `per_file_budget_chars` is kept for backwards compatibility, but the implementation slices at byte boundary then backs off to a clean UTF-8 char boundary (≤3 trailing bytes dropped). For pure-ASCII corpus content, byte ≈ char.

3. **cap_total_chars value:** `16000` (entries above) → **`25000`** (shipped). Raised in CR cycle 1 #13 reconciliation: per-file defaults sum to 20000; cap must comfortably exceed sum + headers + framing markers (~500 chars overhead) so the cap path is a true sum-overshoot safety net, not a common-case truncation path. Terminology corrected: "floor" → "ceiling" (a cap is a maximum).

4. **Relationship added:** "No relations added/removed" (entries above) → **InjectionBudget → Hook (user-prompt-submit)** relationship row added to `.sdd/data-model.md`. The hook reads `parameters.injection.per_file_budget_chars` once per turn and applies each file's budget independently; cap_total_chars is the post-truncation defensive ceiling.

These corrections were captured via the per-cycle close-out comments on PR #239 (cycles 1-4) but until this entry, decisions.md still recorded the pre-fix contract. Anyone reading the audit log now sees BOTH the original approval (entries above, dated 08:47Z-10:02Z) AND this correction (dated 18:36Z) — the append-only contract preserves both, with this entry naming the deltas.

## 2026-05-11T13:25Z  [[019-session-start-hook-prints-bootstrap-success-signal-closes-164-bug-4]]  feature/mark-shipped
F019 session-start bootstrap-ready signal shipped via PR #249 / tag `v1.8.2`. Closes #164 bug 4 only. One-line `[SDD bootstrap] ready` cue in `templates/.claude/hooks/session-start.sh` (mirrored to live); fires when `.sdd/` exists + zero `.shipped` markers under work-item folders; suppresses for returning users. 1 BUILD task-test T01 GREEN. Cycle-0 CR (auto-merge no findings). Note: had to manually scaffold as F019 (skipping start.sh's #231 ID-picker because 2 parallel-agent PRs already used F015 — chicken-egg with #247 still in flight as the fix for that ID-picker gap).
