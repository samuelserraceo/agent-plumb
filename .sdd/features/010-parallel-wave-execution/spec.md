...
- **AC4 — Spec.md row isolation via orchestrator-flips.** Wave-task subagents commit only their own test + code files (disjoint sets — no spec.md touch). The orchestrator commits ONE spec.md edit after the wave returns, flipping every wave-task row from `[ ]` to `[x]` at once. Git sees one spec.md edit per wave instead of N concurrent ones — no merge needed. (Original 3-way-merge framing was empirically refuted by T203: adjacent row diff context overlaps and would conflict. Re-approved in `decisions.md` under `feature/data-contract (re-approved)` + `feature/flows (re-approved)`.) {verify-by: T-NNN}
