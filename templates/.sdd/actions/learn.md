...
**Stage the recipe file too.** When you commit this section, `.sdd/patterns.md` MUST be staged in the same commit — the framework refuses the commit otherwise. If `patterns.md` doesn't exist yet, create it. Append the new lessons under a heading shaped `## <work-item-type>: <id>-<slug>` — e.g. `## Feature: 003-auth-retry`, `## Bug: 002-safety-hook-still-blocks-...`, or `## Refactor: 001-extract-helper`.

**Link the lesson back to this work-item.** Inside the appended block, add a `Source: [[<id>-<slug>]]` line — like a footnote pointing back at the feature/bug/refactor folder this lesson came from. Position in the block doesn't strictly matter (the helper that reads patterns.md searches the whole block). The payoff: future sessions can click "where did this lesson come from?" and land on the spec, AND a "who cites this lesson?" lookup returns this feature as a source. The reverse direction — which features link to a given recipe via `[[pattern:<slug>]]` — is handled by `proposed-approach`, not here. Example:
