<!-- ════════════════════════════════════════════════════════════════════
     ...
     ════════════════════════════════════════════════════════════════════ -->
<!-- SDD-MANAGED-START version: 1.5.2 -->
...
**Corpus-signature lock around `synthesise()` (Tier 3, defensive — closes #113):** before reading the `.sdd/` corpus signature that becomes the synthesis cache key, the synthesise integration point MUST acquire a lock; release it after the cache write completes. Use `bash .sdd/scripts/corpus-signature-lock.sh acquire <cache-dir>/.corpus-signature.lock.d` then `... release <same-path>` to bound the read-during-write race. The helper is mkdir-as-lockdir (bash 3.2 / macOS compatible — same shape as `background-while-waiting.sh`); two parallel `synthesise()` calls serialise on the lockdir so the signature read sees a coherent snapshot of `.sdd/`. The 5s acquire ceiling (50 retries × 0.1s) is the upper bound on contention waits; if it exits 1, the caller logs the contention and either retries or surfaces an error. Out of scope for v1.0 graph queries (`get_neighbours` / `get_backlinks` / `search_within`) — they face the same race but get a separate atomic-snapshot fix per #113.
