...
- **`refactor`** — the "clean up code without changing behaviour" journey. 4-section SPEC: scope → regression coverage → approach → minimal-diff verify (halts at SPEC end if the diff isn't actually shrinking). Run via `/start --playbook=refactor "<title>"`.
