...
for f in "$WIRE" "$EDGE"; do
  ...
  if grep -qE '^requires_user_approval:[[:space:]]+true' "$f"; then
    echo "FAIL: $f still declares requires_user_approval: true (must be false in v1.6 — technical/mechanical, no product judgement)"
    exit 1
  fi
  grep -qE '^requires_user_approval:[[:space:]]+false' "$f" || { echo "FAIL: $f does not declare requires_user_approval: false explicitly"; exit 1; }
done
